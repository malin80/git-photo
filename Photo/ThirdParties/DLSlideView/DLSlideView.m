...
- (void)showAt:(NSInteger)index{
    if (oldIndex_ != index) {
        ...
        [vc didMoveToParentViewController:self.baseController];
        oldIndex_ = index;
        oldCtrl_ = vc;
...
- (void)switchTo:(NSInteger)index{
    ...
    if (oldCtrl_ != nil && oldCtrl_.parentViewController == self.baseController) {
        isSwitching_ = YES;
        ...
        [self.baseController addChildViewController:newvc];
        ...
        [newvc willMoveToParentViewController:self.baseController];
        
        [self.baseController transitionFromViewController:oldvc toViewController:newvc duration:0.4 options:0 animations:^{
            newvc.view.frame = nowRect;
            oldvc.view.frame = oldEndRect;
        } completion:^(BOOL finished) {
            [oldvc removeFromParentViewController];
            [newvc didMoveToParentViewController:self.baseController];
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (panToIndex < 0 || panToIndex >= [self.dataSource numberOfControllersInDLSlideView:self]) {
            ...
            if (panToIndex != panToIndex_) {
                ...
                [self.baseController addChildViewController:willCtrl_];
                [willCtrl_ willMoveToParentViewController:self.baseController];
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInDLSlideView:self] && panToIndex_ != oldIndex_) {
            if (fabs(offsetx) > kPanSwitchOffsetThreshold) {
                ...
                [UIView animateWithDuration:animatedTime animations:^{
                    ...
                    if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInDLSlideView:self]) {
                        [willCtrl_ endAppearanceTransition];
                        [willCtrl_ didMoveToParentViewController:self.baseController];
